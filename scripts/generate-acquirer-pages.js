...
const PATH_DESCRIPTIONS = {
  'rest-api':      'Cloud API. PAX terminals for card-present; no terminal required for MOTO.',
  'android-pax':   'Native on-terminal SDK. Code runs on the PAX device.',
  'android-hilite':'Bluetooth. Android app on phone/tablet connects to HiLite (DATECS).',
  'ios-hilite':    'Bluetooth. iOS app connects to HiLite (DATECS).',
  'cordova':       'Native SDK wrapper. One JS API for PAX and HiLite.',
};
...
function buildPage(slug, a) {
  ...
  const pathSelectorTabs = PATHS.map(p =>
    `<TabItem value="${p}" label="${PATH_LABELS[p]}">\n<p className="tab-description">${PATH_DESCRIPTIONS[p]}</p>\n</TabItem>`
  ).join('\n');
  ...
  return [
    ...
    '## Integration path',
    '',
    ':::tip Select your integration path once — all code examples on this page update automatically.',
    ':::',
    '',
    `<Tabs groupId="integration-path" className="integration-path-selector">`,
    pathSelectorTabs,
    '</Tabs>',
    '',
    '## Capabilities',
...
function main() {
  ...
  console.log(`✓ Generated ${count} acquirer pages + static/llms.txt`);
}
