...
| `capturedAmount` | string | Yes | Actual capture amount in smallest currency unit as a string |
| `tipAmount` | string | No | Optional tip amount in smallest currency unit as a string |

**Android SDK** — `preAuthorizationCapture(amount, currency, originalTransactionID)`:
...
| `400` | `3051` | `Already reversed` | Pre-auth was already voided or captured | Check your records before retrying |
