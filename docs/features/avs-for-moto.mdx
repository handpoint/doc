...
| No run of 12 or more digits | both | `Invalid billing.<field>, long digit sequences are not allowed` |

There's no country-specific format check — the same fields have to carry postal codes and addresses from anywhere, so anything within these limits is accepted. The digit-run rule guards against a card number landing in a billing field; separators between digits are stripped before counting, so `4111 1111 1111 1111` is rejected too.
