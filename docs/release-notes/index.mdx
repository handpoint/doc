...
New transaction type: [Pre-Authorization](/acquirers/epi#pre-auth). A pre-authorization charge is a temporary hold placed on a customer's payment card. A pre-authorized transaction can be increased (Pre-Auth Increase), captured (Pre-Auth Capture), or fully released (Pre-Auth Reversal).
...
- [Pre-Authorization](/acquirers/epi#pre-auth) transaction type introduced.
