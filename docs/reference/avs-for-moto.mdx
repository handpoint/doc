...
This feature adds a `Billing` object — a postal code and, optionally, a street address — to a MOTO Sale or Pre-Authorization. Handpoint doesn't validate the address or run any lookup: you (or the cardholder, via the on-device prompt) supply the data, and the gateway forwards it to the acquirer as part of the authorization request.

AVS only applies to MOTO — Sale and Pre-Authorization. It doesn't apply to card-present transactions or MOTO Refund, and MOTO must already be enabled for the merchant (see [Prerequisites](#prerequisites)).
...
This applies to Sale and Pre-Authorization only — AVS is not supported on MOTO Refund.

## Code {#code}

Code samples live with the rest of the MOTO implementation on the EPI acquirer page:

- **Remote Sale (MOTO)** — [EPI page, Remote Sale section](/acquirers/epi#remote-sale) — Android (PAX) and Cloud API tabs show how to attach `Billing` and use `enableAvsFields`.
- **Remote Pre-Authorization (MOTO)** — documented in the same [Remote Sale section](/acquirers/epi#remote-sale) — Pre-Auth accepts the same `billing` / `enableAvsFields` options as Sale.
