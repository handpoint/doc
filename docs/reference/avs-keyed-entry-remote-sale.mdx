---
title: "AVS for Keyed Entry and Remote Sale"
sidebar_label: "AVS for Keyed Entry and Remote Sale"
...
# AVS for Keyed Entry and Remote Sale
...
Code samples live with the rest of the Remote Sale implementation on the EPI acquirer page:

- **Remote Sale** — [EPI page, Remote Sale section](/acquirers/epi#remote-sale) — Android (PAX) and Cloud API tabs show how to attach `Billing` and use `enableAvsFields`.
- **Remote Pre-Authorization** — documented in the same [Remote Sale section](/acquirers/epi#remote-sale) — Pre-Auth accepts the same `billing` / `enableAvsFields` options as Sale.
