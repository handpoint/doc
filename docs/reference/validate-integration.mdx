...
The delegate fires `didGetTransactionResponse` when the result is received. If the app is backgrounded or killed mid-transaction:
...
|---|---|
| Cloud API | Use an HTTP client timeout for initial POST. For polling, set a max poll duration and retry interval (recommended: poll every 2s, give up after 90s, then reverse). |
| Android SDK | Override `deviceLogsReady()` or implement a countdown in your UI thread. Do not call a second operation until `endOfTransaction` fires. |
| iOS SDK | Use a `DispatchWorkItem` or `Timer`. Cancel and reverse if no delegate response. |
...
| Sale-and-tokenize (atomic) | `AUTHORISED` + token returned |
...
| MOTO charge using stored token | `AUTHORISED` without any terminal interaction |
