...
:::tip Before you start: test your edge cases first
The [Testing Edge Cases](/reference/testing-edge-cases) page has step-by-step scenarios (declined, network drop, duplicate reference, card fallback, partial approval, MOTO timeout) with per-platform tabs. Run those before working through this checklist.
:::
...
→ Full implementation details: [Partial Approval guide](/reference/partial-approval)
...
For **merchant capability errors** (pre-auth not enabled, MOTO not enabled, refund not allowed, partial reversal rejected) — including their exact error shapes, which tier enforces each one (gateway vs terminal), and the known ViscusDummy limitations — see [Error Handling Guide — Merchant capability errors](/reference/error-handling-guide#merchant-capability-errors).
