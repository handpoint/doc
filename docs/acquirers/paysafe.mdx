...
PAYSAFE is Handpoint's North American acquirer for card-present transactions. It supports sale, refund, reversal, tip adjustment, Remote Sale (card token), and tokenization. Merchants with Interac enabled additionally support Interac debit processing and Void. Partial reversals are not supported.
