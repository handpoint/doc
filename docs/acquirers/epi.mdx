...
id: epi
title: "EPI"
description: "US, Canada card-present acquirer. Supports: sale, refund, reversal, tip-adjustment, pre-auth, moto, tokenization, batching, pre-auth-capture-reversal, avs-for-moto, fee-mitigation."
geography: "US, Canada"
status: active
...
<FlavorSection description="Back-office · linked to original MOTO sale" paths={["android-pax","cordova","javascript-sdk","windows-sdk","backoffice"]}>
...
```javascript
// Unlinked — terminal shows manual card entry screen, no original transaction needed
const { transactionResult } = hp.moToRefund(1000, 'USD');

// Linked — pass the GUID of the original sale to limit the refund to that amount
// const { transactionResult } = hp.moToRefund(1000, 'USD', 'original-sale-guid');
...
:::note `transactionReference` is not returned at dispatch
For `moToRefund`, `operationStartedResult.transactionReference` is `undefined` when the call is initiated — the terminal hasn't processed the card yet. Once the clerk completes card entry, `transactionResult` resolves with the full result including a unique `transactionID`. Use the **original sale's** `transactionReference` with `hp.getTransactionStatus()` to check refund state.
:::
...
```csharp
// Unlinked — terminal shows manual card entry screen
var op = hapi.MoToRefund(new BigInteger(1000), Currency.USD);

// Linked — pass the GUID of the original sale to limit the refund to that amount
// var op = hapi.MoToRefund(new BigInteger(1000), Currency.USD, "original-sale-guid");
...
| `amount` | string / integer | Yes | Smallest currency unit — `"1000"` = $10.00 |
| `currency` | string | Yes | ISO 4217 code |
| `serial_number` | string | Yes (Cloud API) | PAX terminal serial number |
| `terminal_type` | string | Yes (Cloud API) | Terminal model, e.g. `"PAXA920"` |
| `originalTransactionID` | string | No (SDK) | GUID of the original sale — links the refund to that transaction and limits the refund to its amount |
| `refundOptions` | RefundOptions | No (JS/Windows SDK) | Optional: `customerReference`, `moneyRemittanceOptions` |
| `transactionReference` | string | No (Cloud API) | UUID v4 for idempotency and status queries |
...
<FlavorSection description="On-device · chip, contactless, or magstripe" paths={["cloud-api","android-pax","cordova","javascript-sdk","windows-sdk"]}>
...
<FlavorSection description="On-device · operator keys card number" paths={["cloud-api","android-pax","cordova","javascript-sdk","windows-sdk"]}>
...
<FlavorSection description="No card required — settle the held amount" paths={["android-pax","cordova","javascript-sdk","windows-sdk","backoffice"]}>
...
### Pre-Auth Reversal {#pre-auth-void}

<FlavorSection description="Release the hold without charging" paths={["cloud-api","android-pax","cordova","javascript-sdk","windows-sdk"]}>
...
<FlavorSection description="Cancel a captured pre-auth before settlement" paths={["cloud-api","android-pax","backoffice"]}>
...
### Cygma Token {#procharge}

<FlavorSection description="EPI · Cygma vault — MOTO detokenization by gateway; ISV stays out of PCI scope" paths={["cloud-api","android-pax","android-hilite","ios-hilite","cordova","backoffice"]}>
...
4. **Future charges:** Use the `cardToken` in [Remote Sale (MOTO)](#remote-sale) — Backoffice `POST /moto/sale` or Android SDK `hapi.motoSale()`.
...
| Field | Description |
|---|---|
| `cardToken` | Opaque Cygma token. Store this — use for future MOTO charges |
