...
:::info Acquirer notes
EPI. Partial reversal available for US and Canada — uses the MasterCard flag in ISO 8583. Tip adjustment only available on transactions processed without an on-screen tip (tip-at-table flow).
:::
...
:::info
See [AVS for MOTO](/reference/avs-for-moto) for prerequisites, field reference, and edge cases.
:::
...
EPI supports an optional `billing` object on MOTO Sale — postal code (and optionally street address) are forwarded to the acquirer to reduce CNP fraud risk. Requires `avsForMoto` enabled per merchant by Handpoint. See [AVS for MOTO](/reference/avs-for-moto) for prerequisites, field reference, and edge cases.
...
<FlavorSection description="EPI · proCharge vault — MOTO detokenization by gateway; ISV stays out of PCI scope" paths={["cloud-api","android-pax","android-hilite","ios-hilite","cordova"]}>
...
4. **Future charges:** Use the `cardToken` in [Remote Sale (MOTO)](#moto-sale) — Cloud API `POST /moto/sale` or Android SDK `hapi.motoSale()`.
