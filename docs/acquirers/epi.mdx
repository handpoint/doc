...
id: epi
title: "EPI"
description: "US, Canada card-present acquirer. Supports: sale, refund, reversal, tip-adjustment, pre-auth, moto, tokenization, batching, pre-auth-capture-reversal, avs-for-moto."
geography: "US, Canada"
status: active
...
**Geography:** US · Canada &nbsp;·&nbsp; **Cards:** <img src="/img/cards/visa.svg" alt="Visa" title="Visa" height="22" style={{verticalAlign:'middle',marginRight:'4px'}}/><img src="/img/cards/mastercard.svg" alt="Mastercard" title="Mastercard" height="22" style={{verticalAlign:'middle',marginRight:'4px'}}/><img src="/img/cards/amex.svg" alt="Amex" title="American Express" height="22" style={{verticalAlign:'middle',marginRight:'4px'}}/><img src="/img/cards/discover.svg" alt="Discover" title="Discover" height="22" style={{verticalAlign:'middle'}}/>
EPI is Handpoint's primary North American acquirer. It supports the widest feature set across all integration paths — including tip adjustment, partial reversal, pre-authorization, Remote Sale (card token), and deferred tokenization. Typical users are US and Canadian ISVs building retail, restaurant, or healthcare POS systems where merchants need both card-present and card-not-present capabilities from a single acquirer.

:::note Amex partial reversal
Partial reversals are **not supported** on American Express transactions via EPI. Full reversal only.
:::
...
Unlike [Remote Sale](#remote-sale) (which uses a stored card token), key entry sale requires the PAX terminal to be physically present. The operator's system sends the command; the terminal's screen collects the card details.
:::
...
### Key Entry Pre-Authorization {#key-entry-pre-auth}

<FlavorSection description="On-device · operator keys card number for a pre-auth" paths={["cloud-api","android-pax","javascript-sdk","windows-sdk"]}>

Same terminal flow as Key Entry Sale — the cashier types card details on the PAX terminal's touchscreen — but sends a pre-authorization instead of a sale. The authorized hold can be captured or released later.
...
Amount in **smallest currency unit**. The terminal shows a card entry screen.

</TabItem>
<TabItem value="android-pax" label="Android (PAX)">

```kotlin
// Terminal shows manual card entry screen for a pre-authorization
hapi.motoPreauthorization(BigInteger("1000"), Currency.USD)

override fun endOfTransaction(result: TransactionResult, device: Device) {
    if (result.finStatus == FinancialStatus.AUTHORISED) {
        // store result.transactionID for capture or release later
    }
}
```
...
```javascript
const { transactionResult } = hp.preAuthorization(1000, 'USD');
const result = await transactionResult;
if (result.finStatus === 'AUTHORISED') {
  // store result.transactionID for capture or release later
}
...
```csharp
var op = hapi.PreAuthorization(new BigInteger(1000), Currency.USD);
if (!op.OperationStarted) { /* handle connection error */ return; }
...
AVS applies to Key Entry Pre-Authorization the same way it applies to Key Entry Sale. The terminal can prompt for the cardholder's billing postal code after card entry, or you can pass it programmatically via `MoToOptions`.

:::info
See [AVS](/reference/avs) for prerequisites, field reference, and edge cases.
:::
...
| Code | Meaning | Recovery |
|---|---|---|
| `DECLINED` | Issuer declined the hold | Ask cardholder to provide another card |
| `CANCELLED` | Operator cancelled on terminal | No action required |
| `MOTO_NOT_ENABLED` | MOTO not enabled for this merchant | Contact Handpoint team |
...
<FlavorSection description="Back-office · charges a stored card token" paths={["cloud-api","android-pax","cordova","javascript-sdk","windows-sdk"]}>
...
Handpoint back-office MOTO does not accept raw PAN, expiry, or CVV from the ISV. All charges use a `cardToken` issued by a supported provider (e.g. ProCharge, Paysafe, TokenEx). The token provider de-tokenizes at processing time — the ISV never handles card data.
:::


#### Address Verification (AVS)

EPI supports an optional `billing` object on MOTO Sale — postal code (and optionally street address) are forwarded to the acquirer to reduce CNP fraud risk. Requires `avsForMoto` enabled per merchant by Handpoint. See [AVS](/reference/avs) for prerequisites, field reference, and edge cases.
...
```javascript
// Back-office MOTO — pass cardToken, no terminal required
const { transactionResult } = hp.moToSale(1000, 'USD', {
  cardToken: 'YOUR_STORED_CARD_TOKEN'
});
const result = await transactionResult;
if (result.finStatus === 'AUTHORISED') {
...
```csharp
// Back-office MOTO — pass cardToken, no terminal required
var options = new MoToOptions { CardToken = "YOUR_STORED_CARD_TOKEN" };
var op = hapi.MoToSale(new BigInteger(1000), Currency.USD, options);
if (!op.OperationStarted) { /* handle error */ return; }
...
| Field | Type | Description |
|---|---|---|
| `cardToken` | `String?` | Card token for Remote Sale — omit to show terminal entry screen instead |
...
hapi.Sale(new BigInteger(1000), Currency.USD, options);

public void EndOfTransaction(TransactionResult result, Device device)
{
    if (result.FinStatus == FinancialStatus.AUTHORISED) {
        // result.TipAmount — tip chosen; result.TotalAmount — base + tip
    }
}
```
...
| `tipPercentages` | array of integers | Tip percentage options to display, e.g. `[10, 15, 20]` — required |
...
```javascript
// Standalone tokenize — card presented at terminal, no charge
const { transactionResult } = hp.tokenizeCard('USD');
const result = await transactionResult;
if (result.finStatus === 'AUTHORISED') {
...
```csharp
// Standalone tokenize — card presented at terminal, no charge
var op = hapi.TokenizeCard();
if (!op.OperationStarted) { /* handle connection error */ return; }

public void EndOfTransaction(TransactionResult result, Device device)
{
    if (result.FinStatus == FinancialStatus.AUTHORISED) {
        var token = result.CardToken;  // store securely for future MOTO charges
    }
}
```
...
```javascript
// Linked refund
const { transactionResult } = hp.refund(1000, 'USD', '01236fc0-8192-11eb-9aca-ad4b0e95f241');
const result = await transactionResult;
if (result.finStatus === 'AUTHORISED') { /* refund accepted */ }

// Unlinked refund — omit originalTransactionId
const { transactionResult: result2 } = hp.refund(1000, 'USD');
```
...
```csharp
// Linked refund
hapi.Refund(new BigInteger(1000), Currency.USD, "01236fc0-8192-11eb-9aca-ad4b0e95f241");

// Unlinked refund — omit originalTransactionID
hapi.Refund(new BigInteger(1000), Currency.USD);
...
### Remote Refund {#remote-refund}

<FlavorSection description="Back-office · linked to original Remote Sale" paths={["cloud-api","android-pax","cordova"]}>

A **card-not-present refund** submitted directly to the gateway — no terminal or card reader required. References the original Remote Sale by its GUID. Valid up to **12 months** after the original sale date.
...
1. **US and Canada merchants only.** Requires TMS enablement per merchant — contact your Handpoint integration engineer to enable partial reversal for a specific merchant account.
...
| After batch close | Close the batch (`POST /batch/close`), attempt reversal — verify `finStatus: "DECLINED"` and `UNABLE_TO_FIND_MESSAGE_TO_REVERSE` in the polled result; route to Refund |
...
### proCharge Token {#procharge}

<FlavorSection description="EPI · proCharge vault — detokenization by gateway; ISV stays out of PCI scope" paths={["cloud-api","android-pax","android-hilite","ios-hilite","cordova"]}>
...
3. **Deferred tokenization:** You can retrieve a token from a previously completed transaction that was not tokenized at the time — no card re-swipe needed (Cloud API) or requires the PAX device to be connected (Android SDK 7.1013.1+).
4. **Future charges:** Use the `cardToken` in [Remote Sale](#remote-sale) — Cloud API `POST /moto/sale` or Android SDK `hapi.motoSale()`.
...
**Get Card Token — retrieve token from a previous transaction (no terminal required):**
```http
...
:::note Cancelled / reversed transactions
A token can be retrieved from a cancelled or reversed transaction — the card is tokenized during the EMV read, before the authorization outcome. On a **partial approval that the cardholder declines** (terminal auto-reverses the partial charge), the final polled result has `finStatus: "CANCELLED"` and `transactionID` of the auto-reversal. Calling `/token` with the reversal's ID returns error `3112`. Instead, use `originalEFTTransactionID` from the polled result — that is the original SALE's ID and is eligible for deferred tokenization.
:::
...
**Get Card Token — Android SDK** (`hapi.deferredTokenization(eftTransactionID)`):
...
**Get Card Token — Cloud API** (`GET /transactions/{transactionID}/token`):
...
| Field | Description |
|---|---|
| `cardToken` | Opaque proCharge token. Store this — use for future [Remote Sale](#remote-sale) charges |
...
| Code | Meaning | Recovery |
|---|---|---|
| `3112` | Transaction type is not eligible for deferred tokenization | Pass the SALE `transactionID`, not a reversal's. For a partial approval → cancel, use `originalEFTTransactionID` from the polled result, or the `type: "SALE"` entry in `/status/all` |
...
#### When to use it
Use when you need to settle at a specific time rather than waiting for automatic batch close. Most EPI merchants use automatic batch close (~11pm EST) — manual close is only needed for specific settlement timing requirements.

#### Implementation notes

1. **EPI only.** EU acquirers use automatic settlement and do not require manual batch close.
2. After batch close, tip adjustments and reversals are no longer possible — only post-settlement refunds.
3. Auto-close is the default at approximately 11pm EST. Manual close is an alternative, not an addition.
