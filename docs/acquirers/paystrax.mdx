...
<AcquirerPageHeader currentSlug="paystrax" />
...
Paystrax is a European acquirer with a feature set equivalent to EmerchantPay — pre-authorization, Remote Sale (card token), and tokenization — without money remittance support. Typical users are EU ISVs whose merchants are boarded through Paystrax rather than EmerchantPay.
...
4. **Settlement is automatic.** Paystrax settles automatically — there is no manual batch close. The reversal window closes when the acquirer's automatic settlement runs.
