...
- Fixed: MoTo transactions that initially returned an UNDEFINED result now correctly recover the approved transaction result via the Get Transaction Status service.
...
- `Partial Voids` (also known as Partial Reversals) are now supported for *TSYS US* acquirer. Please see [Handling Partial Voids with TSYS US](https://handpoint.atlassian.net/wiki/spaces/PD/pages/5210701825/Handling+Partial+Voids+with+TSYS+US) for further information on this.
