...
A new ***cardPresent*** flag is supported, which comes from our [REST API](/restapi/restintroduction), or its related Cloud clients [JavaScript SDK](/javascript/javascriptintroduction) and [Windows SDK](/windows/windowsintroduction). This new flag allows to indicate the payments flow that a Reversal operation will imply an actual present card. Please see TODO IRU for further information on the purpose of this flag.
